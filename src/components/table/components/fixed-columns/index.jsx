...
class FixedColumnsList extends Component {
  ...
  toggleItem = (item) => {
    ...
    onChange && onChange(this.props.options, item);
    // }
  }
  ...
  Options = () => {
    ...
    return <ul className="I-select-body">
      {options.length ? options.map((item, index) => <li title={item.name}
                                                         ...
                                                         className={item.isFixed ? 'I-chosen' : ''}
          >
            <span className={`I-item-checkbox ${item.isFixed ? 'I-checked' : ''}`}/>
            {item.name}
...
export default FixedColumnsList;
